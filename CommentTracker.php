...
class CommentTracker
{
    ...
    private $db;
    private $my;
    private $del;  // there's no big need to have these vars
    ...
    function generate_comment($c_id, $c_pid, $c_name, $c_text, $c_level ){ // shitty things here
        ...
        '<div class="row comm" style="margin-left:'.$c_margin.'px">'.      // level-based margin
                '
                <div class="col-xs-1">
                    <p align="right"><b>'.$c_name.':</b></p>' . // put name
                '</div>
                <div class="col-xs-4">
                    '.$c_text. ''.                              // put text
                    '<div >
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-xs-6">
                                            <div align="right">
                                                ...
                                                '<input class="hidden" name="c_level" value="'.$c_level.'">'. // that's how we can get LEVEL of comment in future NOT INVOLVED
                 ...
                 <div class="col-xs-1">
                    <form class="form-inline comm-del" action="delete.php" method="GET">
                            <input class="hidden" name="c_del" value="'.$c_id.'">'.     // comment ID for deletion in future
    ...
    function show(){                                   // show list of comments
        ...
        try{
            ...
            while($row=$q->fetch_assoc()){
                $this->generate_comment($row['ID'], $row['PARENT_ID'], $row['NAME'], $row['TEXT'], $row['LEVEL']); // make some html
                $this->do_magic($row['ID']); // we need this to show child comments correctly
            }
    ...
    function delete($c_id){ // delete comment
        ...
        $this->del=0;            // set them to default
        $this->stop=0;           // *
        $this->disconnect();     // close connection
    }

    function find_last($id){                                    // function for search the last child of parent comment to start deleting
    ...
    function del_magic($id){                                    // function for recursive deletion comments
        if($id) {                                               // *
